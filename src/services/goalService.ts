...
import { assertValid, goalSchema, uuidSchema } from '@/lib/validation';
...
export const goalService = {
  async getAll(): Promise<FinancialGoal[]> {
    ...
    return (data || []).map((item) => ({ ...item, target_amount: Number(item.target_amount) }));
  },
  ...
  async getCurrentMonthGoal(): Promise<FinancialGoal | null> {
    ...
    if (error) throw new Error('Erro ao carregar meta atual');
  ...
  async upsert(targetAmount: number, month: number, year: number): Promise<FinancialGoal> {
    assertValid(goalSchema, { targetAmount, month, year });
    ...
    const { data, error } = await supabase
      .from('financial_goals')
      .insert({ user_id: user.id, target_amount: targetAmount, month, year })
      .select()
      .single();
